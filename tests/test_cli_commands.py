...
class TestCliStartRestart:
    ...
    def test_start_calls_daemon_start(self):
        """The 'start' action must call DaemonManager.start()."""
        from llmport.cli import main
        source = inspect.getsource(main)
        assert ".start()" in source or "dm.start()" in source, (
            "'start' action should invoke daemon.start()"
        )

    def test_restart_calls_daemon_restart(self):
        """The 'restart' action must call DaemonManager.restart()."""
        from llmport.cli import main
        source = inspect.getsource(main)
        assert ".restart()" in source or "dm.restart()" in source, (
            "'restart' action should invoke daemon.restart()"
        )
