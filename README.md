...
- 多供应商 API key 明文分文件存储（secrets.yaml, 0600,不进 keychain）
- 按优先级 fallback
...
llmport provider add     # 添加/更新供应商（API key 明文存 secrets.yaml）
...
首次使用先 `llmport setup`（初始化配置目录）或直接 `llmport provider add`。添加供应商时 API key 明文存入 `secrets.yaml`（0600），不写进 `config.yaml`；没配置供应商时 `start` 会拒绝并提示先配置。
...
```
config.yaml    # 可读、可手编辑（网关 / 供应商 / 模型映射,不含 API key）
secrets.yaml   # 明文 API key vault（0600）
```
...
# 供应商：连接信息（api_key 存在 secrets.yaml，不写在这里）
...
控制 API 只保留只读 status + 生命周期。供应商/模型/网关配置统一走 CLI（写 `config.yaml` + `secrets.yaml`，守护进程在运行则自动重启生效），不再暴露运行时写接口——避免程序化注入任意 `base_url` 的 SSRF 面。
